...
#include "SerializableFunctionFields.h"
    ...
    HRESULT AddFunction(BufferBuilderList & builder, ParseableFunctionInfo * function, SRCINFO const * srcInfo, bool skipNestedFunctions = false)
    {
        ...
        if (deferredStubs != nullptr && (attributes & FunctionInfo::Attributes::DeferredParse) != 0)
        {
...
#include "SerializableFunctionFields.h"
    ...
    HRESULT AddTopFunctionBody(FunctionBody * function, SRCINFO const * srcInfo, bool skipNestedFunctions = false)
    {
        topFunctionId = function->GetLocalFunctionId();
        return AddFunction(functionsTable, function, srcInfo);
    }

    HRESULT AddDeferredStubs(DeferredFunctionStub* deferredStubs)
    {
        ...
        PrependUInt32(functionsTable, _u("Character Min"), deferredStubs->ichMin);
        PrependUInt32(functionsTable, _u("Function flags"), deferredStubs->fncFlags);
        PrependStruct(functionsTable, _u("Restore Point"), &deferredStubs->restorePoint);

        // Add all the captured name ids
        PrependUInt32(functionsTable, _u("Captured Name Count"), deferredStubs->capturedNameCount);
        for (uint i = 0; i < deferredStubs->capturedNameCount; i++)
        {
            PrependInt32(functionsTable, _u("Captured Name"), deferredStubs->capturedNameSerializedIds[i]);
        }

        PrependUInt32(functionsTable, _u("Nested Count"), deferredStubs->nestedCount);
        for (uint i = 0; i < deferredStubs->nestedCount; i++)
        {
            AddDeferredStubs(&deferredStubs->deferredStubs[i]);
        }
...
public:
    ...
    HRESULT ReadFunctionBody(const byte * functionBytes, FunctionProxy ** functionProxy, Utf8SourceInfo* sourceInfo, ByteCodeCache * cache, NativeModule *nativeModule, bool deserializeThis, bool deserializeNested = true, Js::DeferDeserializeFunctionInfo* deferDeserializeFunctionInfo = NULL, bool skipNestedFunctions = false)
    {
...
#include "SerializableFunctionFields.h"
    ...
    HRESULT ReadTopFunctionBody(Field(FunctionBody*)* function, Utf8SourceInfo* sourceInfo, ByteCodeCache * cache, bool allowDefer, bool skipNestedFunctions, NativeModule *nativeModule)
    {
        ...
        auto result = ReadFunctionBody(topFunction, (FunctionProxy **)&functionBody, sourceInfo, cache, nativeModule, true, !allowDefer /* don't deserialize nested if defer is allowed */, nullptr, skipNestedFunctions);
...
void TranslateDeferredStubTree(DeferredFunctionStub* deferredStubs, uint stubsCount, ByteCodeBufferBuilder *builder, Recycler *recycler)
{
    if (deferredStubs == nullptr || stubsCount == 0)
    {
        return;
    ...
    for (uint i = 0; i < stubsCount; i++)
    {
        ...
        if (capturedNames != nullptr && capturedNames->Count() != 0)
        {
            ...
            while (iter.IsValid())
            {
                ...
                deferredStubs[i].capturedNameSerializedIds[j] = builder->GetIdOfString(pid->Psz(), pid->Cch());
                iter.MoveNext();
                ++j;
            }
        }
        ...
        TranslateDeferredStubTree(deferredStubs[i].deferredStubs, deferredStubs[i].nestedCount, builder, recycler);
    }
}
...
HRESULT ByteCodeSerializer::SerializeToBuffer(ScriptContext * scriptContext, ArenaAllocator * alloc, DWORD sourceByteLength, LPCUTF8 utf8Source, FunctionBody * function, SRCINFO const* srcInfo, bool allocateBuffer, byte ** buffer, DWORD * bufferBytes, DWORD dwFlags)
{
    ...
    bool serializeParserState = (dwFlags & GENERATE_BYTE_CODE_PARSER_STATE) != 0;
    ...
    if (serializeParserState)
    {
        DeferredFunctionStub *deferredStubs = function->GetDeferredStubs();
        TranslateDeferredStubTree(deferredStubs, function->GetNestedCount(), &builder, scriptContext->GetRecycler());
    }
...
HRESULT ByteCodeSerializer::DeserializeFromBufferInternal(ScriptContext * scriptContext, uint32 scriptFlags, LPCUTF8 utf8Source, ISourceHolder* sourceHolder, SRCINFO const * srcInfo, byte * buffer, NativeModule *nativeModule, Field(FunctionBody*)* function, uint sourceIndex)
{
    ...
    hr = reader->ReadTopFunctionBody(function, sourceInfo, cache, ((scriptFlags & fscrAllowFunctionProxy) == fscrAllowFunctionProxy), false, nativeModule);
